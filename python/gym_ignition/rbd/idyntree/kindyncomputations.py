...
class KinDynComputations:
    ...
    def set_robot_state(self,
                        ...
                        world_gravity: np.ndarray = np.array([0, 0, -9.806])) -> None:
        ...
        if world_gravity.size != 3:
            raise ValueError(world_gravity)
        ...
        world_gravity_idyntree = \
            numpy.FromNumPy.to_idyntree_fixed_vector(array=world_gravity)
    ...
    def get_joint_positions(self) -> np.ndarray:
        ...
        ok = self.kindyn.getJointPos(vector)
    ...
    def get_joint_velocities(self) -> np.ndarray:
        ...
        ok = self.kindyn.getJointVel(vector)
    ...
    def get_model_velocity(self) -> np.ndarray:
        ...
        ok_nu = self.kindyn.getModelVel(nu)
    ...
    def get_relative_transform_explicit(self,
        ...
        AB_H_CD = ref_H_other
        # print("->", AB_H_CD.asHomogeneousTransform().toNumPy())
        ...
        if frame_orientation == "world":
            ...
            AB_H_CD = AB_H_CW
            # print(C_H_CW.asHomogeneousTransform().toNumPy())
            # print("->", AB_H_CW.asHomogeneousTransform().toNumPy())
            # print("->", AB_H_CD.asHomogeneousTransform().toNumPy())
        ...
        if ref_frame_orientation == "world":
            ...
            AB_H_CD = AW_H_CD
            # print("->", AW_H_A.asHomogeneousTransform().toNumPy())
            # print("->", AB_H_CD.asHomogeneousTransform().toNumPy())

        # return ref_H_other.asHomogeneousTransform().toNumPy()
        # return ref_H_other.asAdjointTransformWrench().toNumPy()
        # print("->", AB_H_CD.asHomogeneousTransform().toNumPy().tolist())
        # print("~>\n", AB_H_CD.asHomogeneousTransform().toNumPy())
    ...
    def get_frame_jacobian(self, frame_name: str) -> np.ndarray:
        ...
        # frame_index = self.kindyn.getFrameIndex(frame_name)

        if self.kindyn.getFrameIndex(frame_name) < 0:
        # if frame_index < 0:
            raise ValueError(f"Frame '{frame_name}' does not exist")

        J = idt.MatrixDynSize(6, self.dofs + 6)  # TODO: open PR?

        # Error:
        # python /home/dferigo/.config/JetBrains/PyCharmCE2020.1/scratches/scratch_8.py: /usr/include/eigen3/Eigen/src/Core/Block.h:146: Eigen::Block<XprType, BlockRows, BlockCols, InnerPanel>::Block(XprType&, Eigen::Index, Eigen::Index, Eigen::Index, Eigen::Index) [with XprType = Eigen::Map<Eigen::Matrix<double, -1, -1, 1> >; int BlockRows = -1; int BlockCols = -1; bool InnerPanel = false; Eigen::Index = long int]: Assertion `startRow >= 0 && blockRows >= 0 && startRow <= xpr.rows() - blockRows && startCol >= 0 && blockCols >= 0 && startCol <= xpr.cols() - blockCols' failed.

        ok_J = self.kindyn.getFrameFreeFloatingJacobian(frame_name, J)
        # ok_J = self.kindyn.getFrameFreeFloatingJacobian(1, J)
        # print(ok_J)
        # raise
    ...
    def get_linear_angular_momentum_jacobian(self) -> np.ndarray:
        ...
        ok_J_mom = self.kindyn.getLinearAngularMomentumJacobian(J_mom)
    ...
    def get_centroidal_total_momentum_jacobian(self) -> np.ndarray:
        ...
        ok_J_cmm = self.kindyn.getCentroidalTotalMomentumJacobian(J_cmm)
